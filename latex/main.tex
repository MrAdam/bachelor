...
\chapter*{Indledning}
Fitts' Lov beskriver sammenhængen mellem en motorisk opgaves fart og præcision. Den blev første gang beskrevet af Paul M. Fitts i \cite{fitts1954} i 1954. Her udførte Fitts' tre eksperimenter, hvor 16 testpersoner udførte simple bevægelsesopgaver. Fitts brugte resultaterne fra de tre eksperimenter til at finde en formel, som beskriver tiden det tager at udføre en opgave i forhold til opgavens sværhedsgrad, beskrevet som \textit{}Index of Difficulty}

Udformningen af nærmest alle bruger-interfaces kan forbedres af intuitive matematiske modeller for det menneskelige motoriske system. Forholdet mellem bevægelsers fart og præcision blev første gang påvist af Paul M. Fitts i \cite{fitts1954} i 1954. Siden da har flere udgivelser prøvet at forbedre den originale formel, blandt andet \cite{crossman1957}, \cite{welford1968}, \cite{meyer1988}, \cite{mackenzie1992} og \cite{accot1997}.\\




I 1954 udgav Paul M. Fitts \cite{fitts1954} som søgte at finde informationskapaciteten af det menneskelige motoriske system. Fitts så et behov for et samlende begreb for den menneskelige motoriske kapacitet, grundet de tydelige problemer med at forene mange af de fakta, der var blevet beskrevet i tidligere litteratur. Hans forklaring var, at der var blevet overset en overordnet sammenhæng mellem amplituden, varigheden og variabiliteten i bevægelserne ved motoriske opgaver.\\
Siden udgivelsen af \cite{fitts1954} har der været flere forslag til forbedringer af den originale formel, blandt andet beskrevet i \cite{}


\section*{Noter}
Længere indledning! - meget længere.
Heilmeiers katekisme - tre-fire første punkter passer perfekt til indledning.
Skrive hvorfor vi gør det, hvad det kan bruges til.
Hvad vi gør, hvordan gør vi det anderledes?
\begin{itemize}
...
I 1954 udgav Paul M. Fitts en artikel\footcite{fitts1954} som søgte at finde informationskapaciteten af det menneskelige motoriske system - og dermed en generel formulering for sammenhængen mellem distancen til og størrelsen af et mål i en motorisk opgave og den tid det tager at udføre opgaven. 
...
\begin{minipage}{\linewidth}
	\begin{minipage}[b]{.50\linewidth}
		...
		\captionof{figure}{Fitts' første eksperiment \cite{fitts1954}}
		\label{fig:FittsEx1}
	\end{minipage}
	\begin{minipage}[b]{0.50\linewidth}
		\includegraphics[width=0.8\textwidth]{images/illustrations/fitt_ex2}
		\captionof{figure}{Fitts' andet eksperiment \cite{fitts1954}}
		\label{fig:FittsEx2}
	\end{minipage}
\end{minipage}
...
\captionof{figure}{Fitts' tredje eksperiment \cite{fitts1954}}
\label{fig:FittsEx3}
\end{figure}
...
\caption{Fitts' data og den lineære kurve for Welfords model.}
...
\centering
\vspace{0pt}
\captionof{figure}{Illustration af pegeopgavens start}
...
\caption{Efter at have klikket på center-cirklen}
...
\caption{Før klik på mål-cirklen}
...
Hvis deltagerne bevæger pegeenheden uden for det canvas som opgaven skal udføres i vil denne del af banen ikke komme med i vores gemte data.
...
For at kunne finde ud af hvilken af de fire lineære modeller der beskriver vores data bedst, har vi valgt at tage udgangspunkt i \cite{wu2011experiments} der refererer til tre modeludvælgelseskriterier som værende almindeligt anvendte. Denne bog er citeret af 1624 forskellige bøger og artikler, hvilket understøtter vores valg af denne som troværdig kilde.\\
I \cite{wu2011experiments} afsnit \textit{1.7 VARIABLE SELECTION IN REGRESSION ANALYSIS} bliver $C_p$, $AIC$ og $BIC$ beskrevet som almindeligt anvendte, og der bliver refereret til Ockhams ragekniv som værende et godt udgangspunkt ved modeludvælgelse, da det oftest er den simpleste model der passer bedst. Dette har vi brugt til selv at vælge $LSE$, da denne er simpel, og derfor er understøttet som metode, af Ockhams ragekniv.
\begin{itemize}
...
For et givent datasæt $x$ og en tilhørende statistisk model vil likelihood funktionen beregne sandsynligheden for at modellens parametre $p$ vil resultere i $x$. Ved maximum likelihood findes de værdier for parametrene $p$, som maksimere likelihood funktionen. Det vil sige de værdier for parametrene $p$, som gør det observerede datasæt $x$ mest sandsynligt. Forskellen imellem AIC og BIC er, at BIC vægter antallet af parametre mere negativt end AIC.\\
...
For at være sikre på, at vores data kan bruges til at sammenligne de fire formuleringer har vi først kigget på vores 10 testdeltagere hver for sig. Vi har fitted vores 10 testdeltageres data fra pegeopgaverne til Fitts lov hver for sig. Tre af disse plots kan ses i figurerne \ref{fig:testdeltager1}, \ref{fig:testdeltager6}, \ref{fig:testdeltager10}.  Det kan på graferne ses hvordan punkterne ligger pænt omkring modellen, angivet ved den blå linje. Men også, at enkelte datapunkter bryder tendensen, specielt på figur \ref{fig:testdeltager6}. 

På baggrund af de affine linjers overenstemmelse med største delen af datapunkterne har vi konkluderet, at vores data godt kan danne grundlag for en analyse af de fire formuleringer af Fitts lov. Vi vil dog fjerne alle datapunkter som afviger med mere end 3 gange standardafvigelsen, $\sigma$, som er givet ved kvadratroden af variansen på datasættet \cite{pearson1894}.

\begin{minipage}{\linewidth}
	\begin{minipage}[b]{.50\linewidth}
		\includegraphics[width=\textwidth]{images/plots/plot_testdeltager_1}
		\captionof{figure}{Testdeltager 1}
		\label{fig:testdeltager1}
	\end{minipage}
	\begin{minipage}[b]{0.50\linewidth}
		\includegraphics[width=\textwidth]{images/plots/plot_testdeltager_6}
		\captionof{figure}{Testdeltager 6}
...
\begin{minipage}{\textwidth}
\centering
\includegraphics[width=.25\textheight,width=0.5\textwidth]{images/plots/plot_testdeltager_10}
\captionof{figure}{Testdeltager 10}
...
Hver af de fire formuleringer af Fitts lov har vi tilpasset til vores data fra de 264 testdeltageres pegeopgaver ved lineær regression ved R's $lm()$ funktion. Til hver af modellerne har vi gjort brug af formuleringernes respektive $ID$ som $x$-koordinat og tiden, $t$, det tog testdeltageren at løse en pågældende pegeopgave. R-koden er at se i listing \ref{lst:function}. Datapunkterne og de fire modeller er vist i figur \ref{fig:welford_affine_line}, \ref{fig:fitt_affine_line}, \ref{fig:meyer_affine_line} og \ref{fig:mackenzie_affine_line}. Bemærk, hvordan datapunkterne danner 25 vertikale 'søjler', én for hvert af vores pegeopgavers $ID$.

Det er disse modeller som vores pegeopgaveanalyse vil tage udgangspunkt i. For at finde ud af, hvilken af modellerne der er bedst, vil vi som tidligere beskrevet gøre brug af AIC, da det ikke er muligt ud fra disse figurer at sige noget om.
...
\subsection*{R-kode}
Listing \ref{lst:filter} viser hvordan vi udfiltrerer alle data som ligger udenfor tre standardafvigelser, hvilket for normaltfordelt data vil betyde at vi beholder $99.7\%$ af vores data.
\lstinputlisting[label={lst:filter}, caption=Filter til data, language=R, firstline=24, lastline=26]{analyse.r}
Den relevante R-kode som bliver brugt til analysen, er vist i listing \ref{lst:function}
\lstinputlisting[label={lst:function}, caption=Funktion til modelanalyse, language=R, firstline=33, lastline=51]{analyse.r}
\lstinputlisting[label={lst:analysis}, caption=Kald til analysefunktion, language=R, firstline=53, lastline=62]{analyse.r}
Som det ses af listing \ref{lst:function}, udregner vi først $ID$ for vores data, baseret på de foruddefinerede værdier af $A$ og $W$. Herefter bruger vi $lm()$ til at fitte modellen til vores data, hvorefter vi plotter dataen og modellen i et residualplot.\\

\addcontentsline{toc}{subsection}{AIC-analyse}
\subsection*{AIC-analyse}
%[1] "Fitt's AIC = 75572.3883401724"
%[1] "Welford's AIC = 79415.0662411436"
%[1] "Welford's Additive AIC = 75566.8402100114"
%[1] "MacKenzie's AIC = 75567.185218973"
%[1] "Meyer's AIC = 75620.1633109145"
% TODO: Indsæt tabel med de relative størrelser i stedet for plot
Figur \ref{tab:table_analysis_aic} viser AIC for de fire modeller. De fire modellers AIC er høje, men det er underordnet da det afhænger af antallet af datapunkter, Det der derimod betyder noget er deres indbyrdes forhold. Det ses af tabellen at Meyer's har den laveste AIC og derfor er den model der passer bedst til den indsamlede data. MacKenzie's og Fitt's følger herefter med lidt højere AIC værdier, mens Welford's er den klart dårligste af de fire modeller. Vi har valgt at sætte Meyers AIC værdi som nulpunkt og så beregne forskellen til de andre herfra. 

\begin{table}[h]
\centering
\begin{tabular}{ccc}
Model & AIC værdi & Forskel\\
Meyer & 94598.08 & 0 \\
Mackenzie & 94649.26 & 51.18 \\
Fitts & 94694.76 &96.68 \\
Welford & 96060.29 & 1462.21
\end{tabular}
\caption{AIC værdier og indbyrdes forskel med udgangspunkt i Meyers som nulpunkt}
\label{tab:table_analysis_aic}
\end{table}
