...
\addcontentsline{toc}{subsection}{Laboratorieeksperiment}
\section*{Laboratorieeksperiment}
Lorem ipsum..

\addcontentsline{toc}{subsection}{Crowdsourcingeksperiment}
\section*{Crowdsourcingeksperiment}
Lorem ipsum..
...
For at kunne lave model selection skal vi først finde en model for hver af de fire formuleringer af Fitts' Lov. Til dette har vi brugt lineær regression, da formuleringerne kan ses som lineære modeller, hvor ID-leddet ses som x-variablen og tiden som y-variablen. 

Vi overvejede følgende model selections

\begin{itemize}
\item Least Squared Errors
\item Mallow's $C_p$
\item Akaike Information Criterion (AIC)
\item BBayesian Information Criterion (BIC)
\end{itemize}

I least squared errors tages gennemsnittet af afstanden fra punkterne til modellen, for at finde den bedst passende model. Mallow's $C_p$ er baseret på least squared errors, men tager større højde for overfitting ved flere parametre. 
AIC sammenligner samtlige modeller udfra maximum likelihood og antallet af parametre. Når antallet af parametre stiger, så gives en større straf til modellerne. BIC bruger, som AIC, maximum likelihood og antallet af parametre, men giver en større straf for flere parametre end AIC gør.

Af de fire ovenstående modeller bruger vi AIC, da det er den mest udbredte model at bruge. Vi har til vores analyse brugt programmeringssproget R, som også har indbygget AIC funktionalitet. 
